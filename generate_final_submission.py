"""
Generate Final Submission v3.2 - Ensemble Model
Using proper sample submission format
"""
import sys
import os
...
print("="*60)
print("FINAL SUBMISSION v3.2 - ENSEMBLE")
print("="*60)
...
# Prepare hour/location features
...
for location in train_df['view_label'].unique():
    ...
    for hour in range(24):
        ...
        if len(hour_data) >= 5:
            ...
            # Typical congestion at this hour
...
for idx, row in submission_df.iterrows():
    ...
    if match:
        ...
        if (location, estimated_hour) in hour_location_features:
            ...
            feature_row_enhanced = create_advanced_features(feature_row, video_feature_cols)
            
            # Align features
            for col in feature_columns:
                if col not in feature_row_enhanced.columns:
                    feature_row_enhanced[col] = 0
            
            feature_row_enhanced = feature_row_enhanced[feature_columns]
            
            # Ensemble prediction
            try:
                lgbm_proba = models['lgbm'].predict_proba(feature_row_enhanced)
                xgb_proba = models['xgb'].predict_proba(feature_row_enhanced)
                catboost_proba = models['catboost'].predict_proba(feature_row_enhanced)
...
# Save
output_path = os.path.join(config.SUBMISSIONS_DIR, 'v3.2_ensemble_final.csv')
