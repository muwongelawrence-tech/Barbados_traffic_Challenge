"""
PSEUDO-LABELING SUBMISSION v7.5
Use v7.2's high-confidence predictions as additional training data
...
print("="*60)
print("PSEUDO-LABELING SUBMISSION v7.5")
print("="*60)
...
# STEP 1: Train initial model to get confidence scores
print("\n" + "="*60)
...
# STEP 2: Generate pseudo-labels for sample submission
print("\n" + "="*60)
print("STEP 2: GENERATING PSEUDO-LABELS")
print("="*60)

pseudo_data = []
confidence_threshold = 0.75  # Only use predictions with >75% confidence
...
for idx, row in sample_df.iterrows():
    ...
    if match:
        ...
        if rating_type == 'enter':  # Only pseudo-label ENTER (we have good model for this)
...
print(f"✓ Generated {len(pseudo_df)} high-confidence pseudo-labels (threshold={confidence_threshold})")

if len(pseudo_df) > 0:
    print("\nPseudo-label distribution:")
    print(pseudo_df['congestion_enter_rating'].value_counts(normalize=True).sort_index())

# STEP 3: Combine original + pseudo-labeled data
print("\n" + "="*60)
...
print(f"Combined training data: {len(combined_df)} ({len(train_enhanced)} original + {len(pseudo_df)} pseudo)")
...
# Train EXIT model (no pseudo-labeling for exit)
...
output_path = 'submissions/v7.5_pseudo_labeling.csv'
...
print(f"✓ Total training data: {len(combined_df)}")
