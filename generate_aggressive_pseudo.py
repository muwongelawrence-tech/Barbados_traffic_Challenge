"""
AGGRESSIVE PSEUDO-LABELING v7.6
Lower confidence threshold + more training data
...
print("="*60)
print("AGGRESSIVE PSEUDO-LABELING v7.6")
print("="*60)
...
print("\n" + "="*60)
print("ITERATIVE PSEUDO-LABELING (2 ROUNDS)")
print("="*60)

current_X = X.copy()
current_y = y_enter_encoded.copy()
current_df = train_enhanced.copy()
...
for round_num in range(2):
    ...
    # Generate pseudo-labels with LOWER threshold
    confidence_threshold = 0.70 - (round_num * 0.05)  # 0.70, 0.65
    ...
    if len(pseudo_df) > 0:
        ...
        combined_df = pd.concat([
            current_df[['view_label', 'signaling', 'hour', 'time_segment_id', 
        ...
        current_X, _, _ = create_features(combined_df)
        current_y = le_target.fit_transform(combined_df['congestion_enter_rating'])
        current_df = combined_df
        
        print(f"Total training data: {len(current_df)}")

print(f"\n✓ Final training data: {len(current_df)} samples")
...
for fold, (train_idx, val_idx) in enumerate(skf.split(current_X, current_y)):
    X_train, X_val = current_X.iloc[train_idx], current_X.iloc[val_idx]
    y_train, y_val = current_y[train_idx], current_y[val_idx]
...
output_path = 'submissions/v7.6_aggressive_pseudo.csv'
...
print("\n" + "="*60)
print("AGGRESSIVE PSEUDO-LABELING SUMMARY")
print("="*60)
print("✓ 2 rounds of iterative pseudo-labeling")
print("✓ Lower confidence thresholds (0.70, 0.65)")
print(f"✓ Final training data: {len(current_df)}")
