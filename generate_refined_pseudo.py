"""
REFINED PSEUDO-LABELING v7.8
Fine-tune v7.5's winning approach with:
1. Slightly more training data (confidence 0.73 instead of 0.75)
2. Better feature engineering
3. Optimized hyperparameters
Expected: 0.59 → 0.62-0.67
"""
import pandas as pd
import numpy as np
...
print("="*60)
print("REFINED PSEUDO-LABELING v7.8")
print("="*60)
...
def create_features(df):
    ...
    features['congestion_x_flow'] = features['congestion_proxy'] * features['flow_rate_proxy']
    features['motion_std_x_occupancy_std'] = features['motion_std'] * features['occupancy_std']
...
initial_model = lgb.LGBMClassifier(
    n_estimators=420,  # Slightly more than v7.5
    learning_rate=0.019,  # Slightly lower
    max_depth=8,
    num_leaves=55,  # Slightly more
...
# STEP 2: Generate pseudo-labels with SLIGHTLY lower threshold
print("\n" + "="*60)
print("STEP 2: GENERATING PSEUDO-LABELS")
print("="*60)

confidence_threshold = 0.73  # Between v7.5 (0.75) and v7.6 (0.70)
...
for idx, row in sample_df.iterrows():
    ...
    if match:
        ...
        if rating_type == 'enter':
            ...
            test_features['congestion_x_flow'] = test_features['congestion_proxy'] * test_features['flow_rate_proxy']
            test_features['motion_std_x_occupancy_std'] = test_features['motion_std'] * test_features['occupancy_std']
            ...
            if max_proba >= confidence_threshold:
                ...
                pseudo_data.append({
                    'view_label': location,
                    'signaling': estimated_signaling,
                    'hour': estimated_hour,
                    'time_segment_id': segment_id,
                    'congestion_enter_rating': pseudo_label,
                    'motion_mean': test_features['motion_mean'].iloc[0],
                    'motion_std': test_features['motion_std'].iloc[0],
                    'motion_max': test_features['motion_max'].iloc[0],
                    'occupancy_mean': test_features['occupancy_mean'].iloc[0],
                    'occupancy_std': test_features['occupancy_std'].iloc[0],
                    'occupancy_max': test_features['occupancy_max'].iloc[0],
                    'vehicle_count_proxy': test_features['vehicle_count_proxy'].iloc[0],
                    'flow_rate_proxy': test_features['flow_rate_proxy'].iloc[0],
                    'congestion_proxy': test_features['congestion_proxy'].iloc[0],
                })
...
print(f"Combined training data: {len(combined_df)}")
...
for idx, row in sample_df.iterrows():
    ...
    if match:
        ...
        test_features = pd.DataFrame([{
            ...
            'signaling': le_sig_new.transform([estimated_signaling])[0],
            'hour': estimated_hour,
            'time_segment_id': segment_id,
            'motion_mean': video_features_df['motion_mean'].median(),
            'motion_std': video_features_df['motion_std'].median(),
...
output_path = 'submissions/v7.8_refined_pseudo.csv'
...
print("\n" + "="*60)
print("v7.8 IMPROVEMENTS OVER v7.5")
print("="*60)
print("✓ Confidence threshold: 0.73 (vs 0.75)")
print("✓ More features (21 vs 18)")
print("✓ Better hyperparameters (420 trees, 55 leaves)")
